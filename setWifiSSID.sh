...
if [ "$yes" = "yes" ];then
	...
	sed -i -e "s/${SSID_old::23}/${SSID_old::14}${SSID}\"/g" -e "s/${PASS_old::23}/${PASS_old::14}${PASS}\"/g" */ex??_*/ex??_*.ino
fi
if [ "$yes" = "reset" ];then
	sed -i -e "/${SSID_old::14}/c${SSID_old}" -e "/${PASS_old::14}/c${PASS_old}" */ex??_*/ex??_*.ino
fi
ls */ex??_*/ex??_*.ino | while read file_ino; do
	echo -n $file_ino" "
